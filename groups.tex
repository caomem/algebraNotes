...
\begin{definicao}[Operação induzida por bijeção]
	...
	$$\text{se }\mho(x)\leftrightarrow g_1,\ \mho(y)\leftrightarrow g_2\ \text{e}\ \mho(z)\leftrightarrow g_1g_2\ \text{então} \ xy = z.$$
	Assim, como $\mho(xy) =\mho(z) = g_1g_2 = \mho(x)\mho(y)$, a propriedade de homomorfismo é respeitada e $\mho$ representa um isomorfismo que mapeia o grupo $S$ no grupo $G$.
\end{definicao}
