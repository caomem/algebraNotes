...
\begin{definicao}[Subgrupo]
	Um subconjunto \(H\) de um grupo \(G\) é chamado de \emph{subgrupo} de \(G\) (e escreve-se $H \subseteq G$) se possuir as seguintes propriedades:
...
\begin{definicao}[Homomorfismo de grupo]
	...
	\[\varphi(ab) = \varphi(a)\varphi(b), \; \forall \; a,b\; \in G.\] Quando isso acontece, dizemos que o mapeamento $\varphi$ \emph{preserva as propriedades da estrutura algébrica de grupo}.
...
\begin{definicao}[Isomorfismo de grupos]
	Dois grupos \(G\) e \(G'\) são ditos \emph{isomórfos} se há um mapeamento \emph{bijetivo} $\varphi: G \longrightarrow G'$ que preserva as propriedades da estrutura algébrica de grupo:
